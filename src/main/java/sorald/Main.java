...
public class Main {
  ...
  public void checkArguments(JSAP jsap, JSAPResult arguments) {
    ...
    for (String ruleKey : arguments.getStringArray(Constants.ARG_RULE_KEYS)) {
      if (Processors.getProcessor(Integer.parseInt(ruleKey)) == null) {
        System.out.println(
            "Sorry, repair not available for rule " + ruleKey + ". See the available rules below.");
        printUsage(jsap);
      }
    }
  ...
  public void initConfig(JSAPResult arguments) {
    List<Integer> ruleKeys =
        new ArrayList<>(
            Arrays.asList(arguments.getStringArray(Constants.ARG_RULE_KEYS)).stream()
                .map(s -> Integer.parseInt(s))
                .collect(Collectors.toList()));
    this.getConfig().addRuleKeys(ruleKeys);
    this.getConfig()
        .setOriginalFilesPath(
    ...
    if (arguments.getFile(Constants.ARG_GIT_REPO_PATH) != null) {
      this.getConfig()
          .setGitRepoPath(arguments.getFile(Constants.ARG_GIT_REPO_PATH).getAbsolutePath());
    }
    this.getConfig()
        .setPrettyPrintingStrategy(
            PrettyPrintingStrategy.valueOf(
                arguments.getString(Constants.ARG_PRETTY_PRINTING_STRATEGY)));
    this.getConfig()
        .setFileOutputStrategy(
            FileOutputStrategy.valueOf(arguments.getString(Constants.ARG_FILE_OUTPUT_STRATEGY)));
      ...
      case SEGMENT:
        {
          ...
          Node rootNode =
              SoraldTreeBuilderAlgorithm.buildTree(this.getConfig().getOriginalFilesPath());
