...
public class Node {
  ...
  private int
      javaFilesNbs; // the number of java files in the current directory and all subdirectories.
  ...
  private List<String>
      javaFiles; // if its not a dir then it should contains a collection of java files, should be
    ...
    while (!nodes4Traverse.isEmpty()) {
      ...
      if (node.getJavaFilesNbs() != 0) {
        if (node.isFileNode()) {
          ...
          System.out.println(
              "Dir path: " + node.getRootPath() + " FilesNbs: " + node.getJavaFilesNbs());
        }
      }
