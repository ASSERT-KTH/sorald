...
public class IteratorNextExceptionProcessor extends SoraldAbstractProcessor<CtMethod> {
   ...
   *     <p>We want to process the next() method of any custom Iterator class which doesn't already
  ...
  public boolean isToBeProcessed(CtMethod candidate) {
    ...
    if (candidate.isOverriding(next)) {
      ...
      CtTypeReference<?> noSuchElementTypeRef =
          getFactory().Code().createCtTypeReference(java.util.NoSuchElementException.class);
  ...
  public void process(CtMethod method) {
    ...
    throwStmnt.setThrownExpression(
        ((CtExpression<? extends Throwable>)
            getFactory()
                .createConstructorCall(noSuchElementClass.getReference(), new CtExpression[] {})));
