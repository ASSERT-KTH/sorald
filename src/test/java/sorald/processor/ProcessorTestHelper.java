...
public class ProcessorTestHelper {
   ...
   * "RULE_KEY_". For example, if the test file is for the rule S2164 (which is related to the check
   * {@link org.sonar.java.checks.MathOnFloatCheck}), the directory name must start with "S2164_" or
   * "2164_". The rest of the directory name doesn't matter, and the test file itself can have any
   * name as long as it ends with the .java file extension. Here's an example of a compliant
   * directory structure, where the Java files are test files for {@link
   * org.sonar.java.checks.MathOnFloatCheck}.
   *
   * <p>2164_MathOnFloat | ---- TestCaseFile.java | ---- OtherTestCaseFile.java
   *
   * @param nonCompliantFile Path to a non-compliant Java file that violates precisely one SonarQube
   *     rule.
  ...
  /**
   * Parse the package for a single Java source file. If there is no package statement in the file,
   * or the file cannot be read for any reason, an empty string is returned instead.
   */
  ...
  private static <T extends JavaFileScanner> Class<T> getCheckClassByKey(String ruleKey) {
    // could use a static lookup table here for efficiency, but the list is so small at this point
    // that it
    ...
    return (Class<T>)
        Constants.SONAR_CHECK_CLASSES.stream()
            ...
            .orElseThrow(
                () ->
                    new IllegalArgumentException("Could not find check class for key " + ruleKey));
  }
  ...
  private static String getRuleKey(Class<? extends JavaFileScanner> checkClass) {
    return Arrays.stream(checkClass.getAnnotationsByType(Rule.class))
        ...
        .findFirst()
        .orElseThrow(
            () -> new IllegalStateException(checkClass.getName() + " does not have a key"));
  }
